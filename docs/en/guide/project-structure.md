...
- **ScheduleContext** — centralized app state via `ScheduleProvider`
- State management split across specialized hooks:
...
The production build uses `vite-plugin-singlefile` to bundle the entire app into a single `index.html` file (~280 KB). This file can be opened directly in a browser without a server.
